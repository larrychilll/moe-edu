  ...
  return (
    ...
    >
      {showCategory && category && (
        <span className={`inline-flex w-fit items-center gap-1 rounded-md border px-2 py-0.5 text-[11px] font-medium ${a.badge}`}>
          <span>{category.iconEmoji}</span>
          {category.name}
        </span>
      )}
      <h3 className="text-[15px] font-semibold leading-snug tracking-tight">{topic.title}</h3>
