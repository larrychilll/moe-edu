  ...
  return (
    ...
    >
      {showCategory && category && (
        <span className={`inline-flex w-fit items-center gap-1 rounded-md border px-2 py-0.5 text-xs font-medium ${a.badge}`}>
          <span aria-hidden>{category.iconEmoji}</span>
          {category.name}
        </span>
      )}
      <h3 className="text-base font-semibold leading-snug tracking-tight">{topic.title.split(/[:：]/)[0]}</h3>
      <p className="text-base leading-7 text-zinc-600 line-clamp-2">{topic.summary}</p>
